...
- **アラート機能**: 価格到達アラートとDiscord通知（無料）
- **問合せ機能**: システムサポートと技術的質問
- **🆕 銘柄アーカイブ機能**: 過去に合致した銘柄の履歴管理と検索
- **🆕 手動スコア評価機能**: AI計算中表示→手動でS,A+,A,B,C評価入力
- **🆕 Discord通知機能**: 合致銘柄の即座通知（完全無料・制限なし）
...
- 売買シグナル自動計算表示
- エントリーポイント最適化アドバイス
- 🆕 手動スコア評価機能（「AI計算中...」→S,A+,A,B,C入力）
- 🆕 過去銘柄アーカイブ表示（検索・フィルタ機能付き）
...
- 🆕 アーカイブ機能: `/api/archive/stocks`, `/api/archive/search`
- 🆕 スコア評価: `/api/scores/manual`, `/api/scores/update`
- 🆕 Discord通知: `/api/notifications/discord`, `/api/discord/settings`
...
| Discord | 即座通知 | https://discord.com | 完全無料・制限なし |
